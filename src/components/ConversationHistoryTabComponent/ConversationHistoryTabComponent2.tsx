...
class ConversationHistoryTabComponent extends React.Component<MyProps, MyState> {
    ...
    async componentDidUpdate() {
        if (this.state.phoneNumber !== this.props.task?.attributes.from) {
            const fetchConversationsRequest = await fetchConversationsByParticipant(this.props.manager, this.state.phoneNumber, process.env.FETCH_CONVERSATIONS_AS_OLD_AS).then((convos) => {
        ...
        return (      
            ...
            <div>
                ...
                <Box padding="space20" width="100vw">
                    ...
                    {
                        Array.isArray(this.state.conversations) && this.state.conversations.map((conversation, index) => {
